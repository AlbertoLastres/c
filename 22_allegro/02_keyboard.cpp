...
void recalcular_posiciones(struct Nave nave[N], struct Barra2 barra2[BARRAS]){
    for (int i=0; i<N; i++){
...
int main(int argc, char **argv){
    ...
    al_register_event_source(mensajes, al_get_keyboard_event_source());
    ...
    while(1) {
        ...
        if (dibujar && al_is_event_queue_empty(mensajes) ){
            ...
            recalcular_posiciones(nave, barra2);
        ...
        if(ev.type == ALLEGRO_EVENT_TIMER) {
            for (int i=0; i<N; i++){
            if(key[KEY_UP] && nave[i].navy >= 4.0) {
                nave[i].navy -= 4.0;
            }
            if(key[KEY_DOWN] && nave[i].navy <= SH - 4.0) {
                nave[i].navy += 4.0;
            }
            if(key[KEY_LEFT] && nave[i].navx >= 4.0) {
                nave[i].navx -= 4.0;
            }
            if(key[KEY_UP] && nave[i].navx <= SW - 4.0) {
                nave[i].navx -= 4.0;
            }
            redraw = true;
        }
